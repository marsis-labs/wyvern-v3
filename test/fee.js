...
contract('WyvernExchange', (accounts) =>
	...
	*/
	...
	// add fee	
	const erc1155_erc20_match_right_static_call_fee = async (maximumFill,fillCount) => 
		{
		let account_a = accounts[0]
		let account_b = accounts[6]
		let account_f = accounts[8]
		...
		await Promise.all([erc20.approve(proxy1,price*maximumFill*2,{from: account_a}),erc1155.setApprovalForAll(proxy2,true,{from: account_b})])
		await Promise.all([erc20.mint(account_a,price*maximumFill*2),erc1155.mint(account_b,tokenId,maximumFill)])
		
		let balanceA = await erc20.balanceOf(account_a)
		console.log("[Fee] balanceA --->", balanceA)
		...
		let feeRate = 0.05
		let aFee = Math.floor(price * feeRate) 
		let aValue = Math.floor(price * (1-feeRate))

		console.log("[Fee] aFee --->", aFee)
		console.log("[Fee] aValue --->", aValue)
 
	
		// a
		const aEDParams = web3.eth.abi.encodeParameters(['address', 'uint256'], [erc20.address, aValue])
		const aEDSelector = web3.eth.abi.encodeFunctionSignature('transferERC20Exact(bytes,address[7],uint8,uint256[6],bytes)')
		
		// fee
		const feeEDParams = web3.eth.abi.encodeParameters(['address', 'uint256'], [erc20.address, aFee])
		const feeEDSelector = web3.eth.abi.encodeFunctionSignature('transferERC20Exact(bytes,address[7],uint8,uint256[6],bytes)')
		...
		['address[]', 'uint256[]', 'bytes4[]', 'bytes'],
		[[statici.address ,statici.address], 
		[(aEDParams.length - 2) / 2, (feeEDParams.length - 2) / 2], 
		[aEDSelector,feeEDSelector],
		 aEDParams + feeEDParams.slice(2)]
		)
		...
		const sig = await exchange.sign(one, account_a)
		const firstERC20Call = erc20c.methods.transferFrom(account_a, account_b, aValue).encodeABI()
		const feeERC20Call = erc20c.methods.transferFrom(account_a, account_f, aFee).encodeABI()

		const firstData = atomicizerc.methods.atomicize(
			[erc20.address, erc20.address],
			[0,0],
			[(firstERC20Call.length - 2) / 2, (feeERC20Call.length - 2) / 2],
			firstERC20Call + feeERC20Call.slice(2)
			).encodeABI()
		...
		let new_balance = await erc1155.balanceOf(account_a, tokenId)
		console.log("[Fee] new_balance --->", new_balance)


		assert.equal(await erc20.balanceOf(account_f), aFee, 'Incorrect balance')

		assert.isTrue(new_balance.toNumber() > 0,'Incorrect balance')
		assert.equal(await erc20.balanceOf(account_b), aValue, 'Incorrect balance')
		}
				
	it('matches erc1155 <> erc20 signed orders, matched right, real static call, add fee',async () => 
		{
			return erc1155_erc20_match_right_static_call_fee(1,1)
		})
